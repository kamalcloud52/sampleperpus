...
// ==================== KTA DATA ====================
const defaultConfig = {
    site_title: "Kumpulan Karya Tulis Arab (KTA)",
...
const CSV_URL = 'https://docs.google.com/spreadsheets/d/e/2PACX-1vQ3z6IarX5I6GlIOXZ_bEx9f9RgNVysXSN4A4FqTzoUIVZb-4T1-GgUQMhfzq2vxSPsJDSFVz8HRnnw/pub?gid=262881156&single=true&output=csv';
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        if (line) {
            ...
            if (values.length >= 1 && values[0]) {
                ...
                const tema = values[5] || '';
                if (tema) allBahasa.add(tema.trim());
...
function initCustomSelect(selectId) {
    ...
    trigger.addEventListener('click', (e) => { e.stopPropagation(); trigger.classList.toggle('active'); optionsContainer.classList.toggle('open'); });
    document.addEventListener('click', (e) => { if (!wrapper.contains(e.target)) { trigger.classList.remove('active'); optionsContainer.classList.remove('open'); } });
...
function populateFilterDropdowns() {
    ...
    jenisSelect.innerHTML = '<option value="">Semua Tahun Pelajaran</option>';
    Array.from(allJenis).sort().forEach(jenis => { const option = document.createElement('option'); option.value = jenis; option.textContent = jenis; jenisSelect.appendChild(option); });
    const bahasaSelect = document.getElementById('filter-bahasa');
    bahasaSelect.innerHTML = '<option value="">Semua Tema</option>';
    Array.from(allBahasa).sort().forEach(tema => { const option = document.createElement('option'); option.value = tema; option.textContent = tema; bahasaSelect.appendChild(option); });
...
function filterData() {
    ...
    filteredData = allData.filter(row => {
        ...
        const tema = row[5] || '';
        const matchesBahasa = !selectedBahasa || tema.trim() === selectedBahasa;
...
function renderTableView(data) {
    ...
    data.forEach(row => {
        ...
        tr.innerHTML = `
            ...
            <td class="px-4 py-3">${tahunPelajaran}</td>
            <td class="px-4 py-3">${jk}</td>
            <td class="px-4 py-3">${nama}</td>
            <td class="px-4 py-3">${tema}</td>
...
function renderGridView(data) {
    ...
    data.forEach(row => {
        ...
        gridItem.innerHTML = `
            ...
            <div class="p-3">
                <div class="flex items-start justify-between gap-2 mb-1"><div class="text-xs text-green-600 font-semibold">${tahunPelajaran}</div><p class="text-xs text-gray-600 line-clamp-1 flex-1 text-right">${jk}</p></div>
                <h3 class="font-bold text-sm mb-1 line-clamp-2">${judul}</h3>
                <p class="text-xs text-gray-500 mb-2">${nama}</p>
                <p class="text-xs text-gray-600 mb-2">Tema: ${tema}</p>
...
document.getElementById('reset-btn').addEventListener('click', () => { document.getElementById('search').value = ''; document.getElementById('filter-jenis').value = ''; document.getElementById('filter-bahasa').value = ''; filterData(); document.querySelectorAll('.custom-select').forEach(el => el.remove()); initCustomSelect('filter-jenis'); initCustomSelect('filter-bahasa'); });
